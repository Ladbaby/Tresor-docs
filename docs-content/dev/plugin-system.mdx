...
- Maps `message_start` to `response.created` + `response.in_progress`
- Maps `content_block_delta` (text) to `response.output_text.delta`
- Maps `content_block_delta` (input_json) to `response.function_call_arguments.delta`
- Maps `message_stop` to `response.completed`
