...
| File | Coverage |
|---|---|
| `store_test.go` | Database open/close, schema creation, seed defaults (3 providers: openai-gpt4o, anthropic-sonnet, anthropic-haiku), `active_downstream` → `match_downstreams` migration |
...
| `write_config_test.go` | YAML round-trip (DB → file → DB), overwrite old data, atomic write (tmp file + rename), format/downstream array fields survive round-trip |
...
| File | Coverage |
|---|---|
| `plugins_test.go` | Custom header injection verification, OpenAI-to-Anthropic request/response transform accuracy, Anthropic-to-OpenAI request/response transform accuracy, streaming transforms for both directions, Responses API plugin transforms (all 4 directions), registry listing (8 plugins) |
...
- Plugin registry contains all built-in plugins (8)
