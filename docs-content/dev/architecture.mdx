...
│   ├── engine.go         # Gateway forwarding, model resolution, auth, auto-translation, retry on empty response
│   ├── pipeline.go       # Transformer orchestration, streaming
│   ├── retry.go          # Empty response detection + retry with exponential backoff
...
│   ├── openai2responses.go # OpenAI Chat Completions → Responses API
│   ├── anthropic2responses.go # Anthropic Messages → Responses API
│   ├── responses2openai.go # Responses API → OpenAI Chat Completions
│   ├── responses2anthropic.go # Responses API → Anthropic Messages
│   ├── anthropic_image_fix.go # Image extraction from tool results
│   ├── anthropic_usage_fix.go # Anthropic usage block normalization (MiniMax-M3 etc.)
│   ├── openai2gemini.go   # OpenAI → Gemini format conversion
│   ├── anthropic2gemini.go # Anthropic → Gemini format conversion
│   ├── gemini2openai.go   # Gemini → OpenAI format conversion
│   ├── gemini2anthropic.go # Gemini → Anthropic format conversion
│   ├── gemini2responses.go # Gemini → Responses API conversion
│   ├── streaming.go      # SSE parsing and chunk transformation
│   └── streaming_test.go # Comprehensive streaming transform tests
│
...
│   ├── config.go         # Runtime config endpoints (proxy mode, auth keys, password, default tab, retry on empty)
│   ├── icons.go          # Model icon serving (public endpoint)
│   ├── icons_admin.go    # Icon index refresh (admin endpoint)
│   └── embed.go          # Embedded web UI via //go:embed
│
├── middleware/            # Cookie-based session auth for admin API
│   └── auth.go
│
├── icons/                 # Model icon fetching, pattern matching, CDN caching
│   ├── fetcher.go        # Icon fetching and caching
│   ├── index.go          # CDN index management
│   ├── patterns.go       # Model-to-icon pattern matching
│   └── ratelimit.go      # Rate limiting for icon fetcher
│
├── inspect/              # Bounded in-memory store for captured request/response bodies
│   └── store.go          # Inspect store for Logs inspector
│
...
│                   │     Supports: OpenAI Chat Completions, Anthropic Messages,
...
└────────┬────────┘
         │
┌────────▼────────┐
│ 8. Retry on     │  ← (if retry_on_empty is enabled)
│    Empty        │     If downstream returns HTTP 200 but produces no
│    Response     │     content (empty choices, thinking-only output,
│                 │     etc.), retry up to 3 times with exponential
│                 │     backoff. Retries are transparent to the client.
└────────┬────────┘
         │
┌────────▼────────┐
│ 9. Response      │  ← Execute response transformers sequentially
│    Pipeline      │     Non-streaming: full body transform
│                   │     Streaming (SSE): event-by-event transform
└────────┬────────┘
         │
┌────────▼────────┐
│ 10. Return to    │  ← Final response to client
│    Client        │
└──────────────────┘
```
...
- **Only retries on HTTP 200 with empty content.** Non-200 responses (4xx, 5xx) are never retried — LLM client SDKs handle their own retry logic for HTTP errors.
- **Maximum 3 retries** with exponential backoff (500ms, 1s, 2s base) and 0–25% random jitter.
- **Streaming responses:** A `bufferedWriter` holds all SSE events until content is confirmed. When the first content-producing line is detected (text, tool calls, etc.), the buffer flushes to the client. If no content arrives, the stream is discarded and retried.
- **Non-streaming responses:** The full body is read, response transformers are applied, then the format-specific empty check runs. Empty bodies are discarded and retried.
- **Empty detection per format:**
  - **OpenAI Chat Completions:** Empty choices, or no text/tool_calls/refusal in both `message` and `delta` fields
  - **Anthropic Messages:** Empty content array, or blocks containing only `thinking` (no `text`, no `tool_use`)
  - **OpenAI Responses:** Empty output array, or message items with no `output_text`
  - **Google Gemini:** Empty candidates, or parts with no `text`/`functionCall` (parts with `thought:true` excluded)
...
**sessions** — Active admin sessions (multi-session support)
