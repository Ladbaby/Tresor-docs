...
│    Translation   │     automatically insert format converter
└────────┬────────┘
         │
┌────────▼────────┐
...
Tresor exposes `/v1/models` and `/models` endpoints that aggregate all known model IDs from downstream `output_model_ids` and alias input/output model IDs. The response is formatted as an OpenAI-style model list, so OpenAI-compatible clients can discover available models through Tresor.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/config` | Get current runtime settings (proxy mode, proxy keys, password status, default tab) |
