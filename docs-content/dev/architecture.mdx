...
All other `/api/*` endpoints require authentication. The admin API uses **cookie-based session authentication**: after logging in via `POST /api/auth/login`, a session cookie (`tresor_token`, Path=/api/, HttpOnly, SameSite=Lax, 365-day expiry) is issued. Authenticated requests send the cookie (for browser/SSE) or use it as a Bearer header. CLI commands use the raw `admin_password` as Bearer token (backwards compat). Session tokens persist across daemon restarts (stored in SQLite). Login is rate-limited (5 attempts per 60s per IP).
