...
2. **Alias Check** — Look for an active alias matching the model; if found, use its downstream and rewrite the model name
...
5. **Transform & Forward** — Run request through configured transformers, then forward to the resolved downstream
6. **Transform Response** — Run the downstream's response through response transformers before returning to the client
