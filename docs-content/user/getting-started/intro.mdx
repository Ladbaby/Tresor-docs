...
- 🔄 **Retry on Empty Response** — Automatically retry downstream requests that return HTTP 200 but produce no content (e.g., thinking-only responses, empty completions). Up to 3 retries with exponential backoff. Transparent to the client.
...
8. 🔄 **Retry on Empty Response** — (if enabled) If the downstream returns HTTP 200 but produces no content, retry up to 3 times with exponential backoff. Retries are transparent to the client.
9. ↩️ **Transform Response** — Run the downstream's response through response transformers before returning to the client
