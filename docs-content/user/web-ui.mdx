...
For **Anthropic Messages** and **OpenAI Responses** requests, the Parsed tab does **not** mirror the raw JSON key order. The sections are emitted in the order the model actually consumes the prompt — i.e. the prompt-cache prefix hierarchy:
...
A small banner with a `?` tooltip appears at the top of the Parsed view for these formats so the order discrepancy is visible. For OpenAI Chat Completions, Gemini, and unrecognised formats the sections keep their raw JSON order, since those formats don't have a server-controlled cache prefix.
  :::
